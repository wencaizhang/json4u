...
export const useEditorStore = create<EditorState>()((set, get) => ({
  ...
  resetHighlight() {
    ...
    // If text compare is enabled, auto re-compare after content settles
    if (getStatusState().enableTextCompare) {
      delayedReCompare();
    }
  },
}));

// Debounced auto re-compare: only fires after user stops typing for 500ms
const delayedReCompare = debounce(() => {
  const { comparer } = getEditorState();
  if (comparer && getStatusState().enableTextCompare) {
    getEditorState().compare();
  }
}, 500);

export function cancelDelayedReCompare() {
